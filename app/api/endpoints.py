...
def post_config(name: str):
    ...
    return flask.make_response({"success": True, "msg": f"{name} is saved!"}), 200

@api.route("/config/test/<name>", methods=["POST"])
def test_config(name: str):
    ...
    command = "/usr/local/webserver/nginx/sbin/nginx -t"
    # 执行命令并获取执行日志
    stdout, stderr = execute_command(command)
    msg = f"stdout:{stdout};stderr:{stderr}"

    return flask.make_response({"success": True, "msg": msg}), 200

@api.route("/config/apply/<name>", methods=["POST"])
def apply_config(name: str):
    ...
    command = "/usr/local/webserver/nginx/sbin/nginx -s reload"
    # 执行命令并获取执行日志
    stdout, stderr = execute_command(command)
    ...
    for _ in os.listdir(config_path):
        ...
        if os.path.isfile(os.path.join(config_path, _)):
            ...
            if state == "conf":
                ...
                sites_available.append({"name": domain, "time": time})
                ...
                sites_available.append({"name": domain.rsplit(".", 1)[0], "time": time})

    # sort sites by name
    sites_available = sorted(sites_available, key=lambda _: _["name"])
    return (
        flask.render_template(
            "domains.html", sites_available=sites_available, sites_enabled=sites_enabled
        ),
        200,
    )
...
def get_domain(name: str):
    ...
    return (
        flask.render_template("domain.html", name=name, file=_file, enabled=enabled),
        200,
    )
